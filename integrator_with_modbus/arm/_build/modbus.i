...
{
    ((NRF_TIMER_Type *) 0x4000A000UL)->EVENTS_COMPARE[0] = 0; 
...
{
    if (((NRF_UART_Type *) 0x40002000UL)->EVENTS_RXDRDY)
    {
        ...
        {
            ((NRF_UART_Type *) 0x40002000UL)->RXD; 
        }
        else
        {
            byte_received();
    ...
    else if (((NRF_UART_Type *) 0x40002000UL)->EVENTS_TXDRDY)
    {
...
void mb_init(void)
{
    ...
    ((NRF_UART_Type *) 0x40002000UL)->PSELTXD = 9;
    ((NRF_UART_Type *) 0x40002000UL)->PSELRXD = 11;
    ...
    do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 117, (uint8_t*) "..\\modbus.c"); } while (0); } } while (0);

    err_code = sd_nvic_SetPriority(UART0_IRQn, NRF_APP_PRIORITY_LOW);
    do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 120, (uint8_t*) "..\\modbus.c"); } while (0); } } while (0);

    err_code = sd_nvic_EnableIRQ(UART0_IRQn);
    do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 123, (uint8_t*) "..\\modbus.c"); } while (0); } } while (0);
...
uint8_t mb_send_write_rsp()
{
    ...
    memcpy((uint8_t *)&g_buffer, &rsp_hdr, 5);
    g_bytes_in_rsp_buffer = 5;
...
uint8_t mb_send_error_rsp(uint8_t code, uint8_t exception)
{
    ...
    memcpy((uint8_t *)&g_buffer, &rsp_hdr, 2);
    g_bytes_in_rsp_buffer = 2;
...
{
    g_buffer[g_rq_idx] = ((NRF_UART_Type *) 0x40002000UL)->RXD; 
    ++g_rq_idx;                          
...
static void packet_timeout_handler(void)
{
    ...
    {
        app_sched_event_put(0, 0, &request_handler);
    }
}
...
static void packet_timeout_timer_init()
{
    ...
    ((NRF_TIMER_Type *) 0x4000A000UL)->BITMODE   = (0x03UL);
    ((NRF_TIMER_Type *) 0x4000A000UL)->PRESCALER = 5;
    ((NRF_TIMER_Type *) 0x4000A000UL)->CC[0]     = 120;
    ((NRF_TIMER_Type *) 0x4000A000UL)->INTENSET  = (0x1UL << (16UL));
...
static uint8_t prepare_id_rsp()
{
    ...
    memcpy((uint8_t *)&g_buffer, &rsp_hdr, 7); 

    
    g_bytes_in_rsp_buffer             = hdr_len;
    g_buffer[g_bytes_in_rsp_buffer++] = 0x00;
    g_buffer[g_bytes_in_rsp_buffer++] = sizeof ("Nordic");
    memcpy((uint8_t *)&g_buffer + g_bytes_in_rsp_buffer, "Nordic", sizeof ("Nordic") - 1); 
    g_bytes_in_rsp_buffer += sizeof ("Nordic") - 1;

    
    g_buffer[g_bytes_in_rsp_buffer++] = 0x01;
    g_buffer[g_bytes_in_rsp_buffer++] = sizeof ("BLE4");
    memcpy((uint8_t *)&g_buffer + g_bytes_in_rsp_buffer, "BLE4", sizeof ("BLE4") - 1); 
    g_bytes_in_rsp_buffer += sizeof ("BLE4") - 1;

    
    g_buffer[g_bytes_in_rsp_buffer++] = 0x02;
    g_buffer[g_bytes_in_rsp_buffer++] = sizeof ("0.1");
    memcpy((uint8_t *)&g_buffer + g_bytes_in_rsp_buffer, "0.1", sizeof ("0.1") -
           1); 
    g_bytes_in_rsp_buffer += sizeof ("0.1") - 1;
...
static uint8_t prepare_read_rsp()
{
    ...
    rsp_hdr.byte_count = 2 * rq_hdr->regs_quantity[1]; 
    ...
    crc = get_CRC((const uint8_t *)g_buffer, g_bytes_in_rsp_buffer);
    memcpy((uint8_t *)&g_buffer + 2 + rsp_hdr.byte_count, &crc, sizeof (crc));
    g_bytes_in_rsp_buffer += sizeof (crc);
...
static void request_handler(void * p_event_data, uint16_t event_size)
{
    ...
    if (exception)
    {
        mb_send_error_rsp(fun + 0x80, exception);
    }
    else
    {
        send_rsp_byte();
...
{
    nrf_gpio_cfg_output(9);
    ((NRF_UART_Type *) 0x40002000UL)->TXD = byte;
}
