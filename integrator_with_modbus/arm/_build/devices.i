...
extern _Bool m_memory_access_in_progress;  

extern mb_bt_memory_t g_memory;
bt_devices_list_t     g_devs_list;
ble_gap_scan_params_t g_scan_param;  
int8_t                g_conn_dev_idx = -1;

extern uint8_t g_buffer[32];
...
static uint8_t find_in_list(ble_gap_addr_t * address, uint8_t * pos)
{
    *pos = 0;
    ...
    for (int i = 0; i < g_devs_list.number && i < 7; ++i)
    {
...
void bt_scan_start(void)
{
    ...
    do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 123, (uint8_t*) "..\\devices.c"); } while (0); } } while (0);
    ...
    g_scan_param.active      = 0;             
    g_scan_param.selective   = 0;             
    g_scan_param.interval    = 0x00A0; 
    g_scan_param.window      = 0x00A0;   
    g_scan_param.p_whitelist = 0;          
    g_scan_param.timeout     = 0x0001;

    err_code = sd_ble_gap_scan_start(&g_scan_param);
    do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 140, (uint8_t*) "..\\devices.c"); } while (0); } } while (0);
}
...
uint32_t bt_connect(void)
{
    ...
    if (rq_packet->byte_count != (7 + 1))
        return ((0x0) + 9);

    dev_idx = start_address / (7 + 1);
...
uint32_t bt_disconnect(const uint16_t conn_handle)
{
    ...
    {
        return sd_ble_gap_disconnect(conn_handle, 0x13);
    }
